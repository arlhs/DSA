...
Node *buildBalancedTree(Node *root)
{
    ...
    // stored inorder sequeance in vector
    ...
    // building tree with element 0 to mid of inorderSeq
    // i.e., building left subtree
    while (start < end)
    {
        int mid = (start + end) / 2;
    ...
    // // building tree with element mid+1 to size-1 of inorderSeq
    // i.e., building right subtree
    while (start < end)
    {
        int mid = (start + end) / 2;
        root1 = Insert(root1, inorderSeq[mid]);
